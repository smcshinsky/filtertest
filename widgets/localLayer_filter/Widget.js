...
define([
 ...
 'dijit/form/Select'
  ...
  function (
    ...
    var defExp = '',
	  ...
	  defExp4 = null,
	  defExp5 = null,
	  defExp6 = null, //added for checkboxes
	  defExp7 = null, //added for checkboxes
      defExp8 = null, //added for checkboxes
	  defExp9 = null, //added for checkboxes
	  defExp10 = null, //added for checkboxes
	  defExp11 = null, //added for checkboxes
	  defExp12 = null, //added for checkboxes
	  defExp13 = null, //added for checkboxes
      fromDate = null,
      toDate = null,
      crimeDate = null,
      filterLayer = null;
 ...
 $("#date").change(function () {
  ...
  //--------------select multiple crimes-----------------
        on(dom.byId("execute"), "click", function(){  
          query("input").forEach(function(node){  
            if(node.id == "Larceny/Theft" && dom.byId(node.id).checked){  
                defExp5 = 'crime =\'LarcenyTheft\'';
			}  
            else if(node.id == "VehicleTheft" && dom.byId(node.id).checked){  
                defExp6 = 'crime =\'Vehicle Theft\'';
            }  
			else if(node.id == "Assult" && dom.byId(node.id).checked){  
                defExp7 = 'crime =\'Assult\'';
            } 
			else if(node.id == "Other" && dom.byId(node.id).checked){  
                defExp8 = 'crime =\'Other\'';
            } 
			else if(node.id == "DUII" && dom.byId(node.id).checked){  
                defExp9 = 'crime =\'DUII\'';
            } 
			else if(node.id == "Fraud" && dom.byId(node.id).checked){  
                defExp10 = 'crime =\'Fraud\'';
            } 
			else if(node.id == "Burglary" && dom.byId(node.id).checked){  
                defExp11 = 'crime =\'Burglary\'';
            } 
			else if(node.id == "TrafficStop" && dom.byId(node.id).checked){  
                defExp12 = 'crime =\'Traffic Stop\'';
            } 
            /*else if(node.id == "Clear" && dom.byId(node.id).checked){  
                defExp13 = '';                                           
            } */              
          })  
          console.log(defExp5 + " AND " + defExp6 + " AND " + 
		  defExp7 + " AND " + defExp8 + " AND " + defExp9 + " AND " + 
		  defExp10 + " AND " + defExp11 + " AND " + defExp12);  
          //lLayer.setDefinitionExpression(defExp5 + " AND " + defExp4);   
        })  
		 ...
		 var defArr2 = [];
            ...
            if (defExp10 !== '') {
              defArr.push(defExp10);
			}
            if (defExp11 !== '') {
              defArr.push(defExp11);
            }
			if (defExp12 !== '') {
              defArr.push(defExp12);
            }
            lLayer.setDefinitionExpression(defArr2.join(' AND '));

  //--------------End select multiple crimes-----------------
